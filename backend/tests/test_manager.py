...
async def test_admin_can_change_user_role(
    ...
    resident_id = next(u["id"] for u in users.json()["items"] if u["email"] == RESIDENT_EMAIL)
...
async def test_admin_cannot_change_own_role(
    ...
    users = await client.get("/manager/users", headers=headers)
    admin_id = next(u["id"] for u in users.json()["items"] if u["email"] == "admin4@example.com")
...
async def test_admin_cannot_delete_own_account(
    ...
    users = await client.get("/manager/users", headers=headers)
    admin_id = next(u["id"] for u in users.json()["items"] if u["email"] == "admin7@example.com")
