...
├── pages/         # login, register, user/index (resident dashboard), manager.vue + manager/* (manager/admin dashboard)
├── components/    # shared UI (FormInput, FormSelect, Modal, Pagination, StatusBadge, ...) + components/manager/*
...
- `useApplications()` — `listOwn`, `create`, `getOwn`, `update` (resident operations) and `listAll`, `approve`, `reject`, `requestChanges`, `revoke` (manager operations).
